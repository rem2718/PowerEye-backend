...
# Fixture to create a default connection and disconnect in teardown
@pytest.fixture(scope='function')
def setup_teardown_mongo_connection(request):
    # Establish a connection
    mongoengine.connect(db='your_database', host='your_connection_string', alias='default')
    ...
    # Optionally, you can return the connection
...
def test_valid_password(setup_teardown_mongo_connection):
    password = "@SmartMaya1"
...
def test_password_length(setup_teardown_mongo_connection):
    password = "maya1!"
...
def test_no_special_character_password(setup_teardown_mongo_connection):
    password = "SmartMaya1"
    response = validate_password(password)
    result, message, status_code = response
    assert result is False
...
def test_empty_password(setup_teardown_mongo_connection):
    password = ""
    response = validate_password(password)
    result, message, status_code = response
    assert result is False
...
def test_no_upper_case_password(setup_teardown_mongo_connection):
    password = "@smartmaya1"
    response = validate_password(password)
    result, message, status_code = response
    assert result is False
...
def test_no_lower_case_password(setup_teardown_mongo_connection):
    password = "@SMARTMAYA1"
    response = validate_password(password)
    result, message, status_code = response
    assert result is False
...
def test_no_digit_password(setup_teardown_mongo_connection):
    password = "@SmartMaya"
    response = validate_password(password)
    result, message, status_code = response
    assert result is False
...
def test_signup_valid_credentials(setup_teardown_mongo_connection):
...
def test_signup_invalid_meross_credentials(setup_teardown_mongo_connection):
    email = "mayakhalide2001@gmail.com"
...
def valid_user_credentials(setup_teardown_mongo_connection):
...
def test_login_missing_email(setup_teardown_mongo_connection):
    response, _ = login('', 'Smarthomemaya#1')
    assert response.status_code == 200
    assert response.json == {'error': 'Email is required.'}

def test_login_missing_password(setup_teardown_mongo_connection):
    response, _ = login('mayakhalide2001@gmail.com', '')
    assert response.status_code == 200
    assert response.json == {'error': 'Password is required.'}

def test_login_invalid_credentials(setup_teardown_mongo_connection):
...
# Fixture to create a user with a notified device
@pytest.fixture
def create_user_with_device():
    user = User(email="test@example.com", password="password", cloud_password= "cloud_password")
    user.save()
    device = NotifiedDevice(device_id="test_device_id")
    user.notified_devices.append(device)
    user.save()
    return user.id, device.device_id

# Test the logout function
def test_logout_with_valid_device(create_user_with_device):
    user_id, device_id = create_user_with_device
    with app.test_request_context():
        response, status_code = logout(user_id, device_id)

    assert status_code == 200


# Test get_user_info function
def test_valid_get_user_info(setup_teardown_mongo_connection):
...
def test_nonexistent_user(setup_teardown_mongo_connection):
    user_id = "0002954c710c09f3b476eece"
    response, status_code = get_user_info(user_id)
    assert status_code == 404

def test_deleted_user(setup_teardown_mongo_connection):
...
# def test_exception_for_signup_handling(setup_teardown_mongo_connection):
#     user_id = "6552954c710c09f3b476eece"
...
def test_update_all_user_info(setup_teardown_mongo_connection):
    user_id = "6552954c710c09f3b476eece"
    meross_password = "smarthomemaya"
    power_eye_password = "Smartmaya1#"
    username = "Mayati"

    # Add this line to print the input values
    print(f"Input values: user_id={user_id}, meross_password={meross_password}, power_eye_password={power_eye_password}, username={username}")

    response, status_code = update_user_info(user_id, meross_password, power_eye_password, username)

    # Add this line to print the actual status code
    print(f"Actual status code: {status_code}")

    assert status_code == 200

# Test updating username and PowerEye password
def test_update_username_and_power_eye_passwords(setup_teardown_mongo_connection):
    user_id = "6552954c710c09f3b476eece"
...
def test_update_PowerEye_password(setup_teardown_mongo_connection):
    user_id = "6552954c710c09f3b476eece"
...
def test_update_username(setup_teardown_mongo_connection):
    user_id = "6552954c710c09f3b476eece"
    username = "maya2001"
...
def test_invalid_PowerEye_password(setup_teardown_mongo_connection):
    user_id = "6552954c710c09f3b476eece"
...
def test_set_goal(user_id, energy, expected_status, expected_message,setup_teardown_mongo_connection):
...
def test_delete_goal(user_id, expected_status, expected_message,setup_teardown_mongo_connection):
...
def test_upload_profile_pic(user_id, file, filename_with_extension, expected_status,setup_teardown_mongo_connection):
...
def test_get_profile_pic(user_id, file_exists, allowed_extensions, expected_status,setup_teardown_mongo_connection):
...
def test_set_fcm_token(user_id, device_id, fcm_token, expected_status, setup_teardown_mongo_connection):
