    ...
    try:
        ...
        if not isinstance(name, str) or len(name) < 2:
            return (
                False,
                jsonify(
                    {"message": "Name should be a string with 2 or more characters"}
                ),
                400,
            )
        ...
        for appliance in user.appliances:
            ...
            if appliance.name == name:
                ...
                return (
                    ...
                    jsonify(
                        {
                            "message": "Name must be unique among all active appliances in your account"
                        }
                    ),
                    400,
                )
        ...
        return (
            False,
            jsonify({"message": f"Error occurred while validating name: {str(e)}"}),
            500,
        )
...
def validate_cloud_id(user_id, cloud_id):
    ...
    try:
        ...
        if cloud_id not in smartplug_ids:
            return (
                False,
                jsonify({"message": "Invalid cloud_id, or the plug is already in use"}),
                400,
            )
        ...
        traceback.print_exc()
        return (
            jsonify({"message": f"Error occurred while validating cloud ID: {str(e)}"}),
            500,
        )
...
def add_appliance(user_id, name, cloud_id, type):
    try:
        ...
        # Validate cloud_id
        is_valid_cloud_id, error_response, status_code = validate_cloud_id(
            user_id, cloud_id
        )
        ...
        appliance = Appliance(
            _id=appliance_id, name=name, type=type, cloud_id=cloud_id, e_type=e_type
        )
        ...
        traceback.print_exc()
        return (
            jsonify({"message": f"Error occurred while adding appliance: {str(e)}"}),
            500,
        )
...
def get_appliance_by_id(user_id, appliance_id):
    try:
        ...
        appliance = next(
            (app for app in user.appliances if str(app._id) == appliance_id), None
        )
        ...
        return (
            jsonify(
                {"message": f"Error occurred while retrieving appliance: {str(e)}"}
            ),
            500,
        )
...
def get_all_appliances(user_id):
    try:
        ...
        return (
            jsonify(
                {"message": f"Error occurred while retrieving appliances: {str(e)}"}
            ),
            500,
        )
    ...
    try:
        ...
        appliance = next(
            (
                ...
                if str(app._id) == str(appliance_id) and not app.is_deleted
            ),
            None,
        )
        ...
        if not rooms_with_appliance:
            return (
                jsonify(
                    {
                        "message": "Appliance deleted, it was not associated with any rooms"
                    }
                ),
                200,
            )
        ...
        traceback.print_exc()
        return (
            jsonify({"message": f"Error occurred while deleting appliance: {str(e)}"}),
            500,
        )
...
def update_appliance_name(user_id, appliance_id, new_name):
    try:
        ...
        # Validate name
        is_valid_name, error_response, status_code = validate_name(
            user_id, new_name, current_appliance_id=appliance_id
        )
        if not is_valid_name:
            return error_response, status_code
        ...
        return (
            jsonify({"message": f"Appliance name updated successfully to {new_name}."}),
            200,
        )
    except DoesNotExist:
        return jsonify({"message": "User not found"}), 404
        ...
        traceback.print_exc()
        return (
            jsonify({"message": f"Error occurred while updating appliance: {str(e)}"}),
            500,
        )
    ...
    try:
        ...
        if not isinstance(status, bool):
            return (
                jsonify(
                    {
                        "error": "Invalid status value. It should be a boolean (True or False)"
                    }
                ),
                400,
            )
        ...
        switched, error_message, status_code = cloud.switch(
            plug_type, cloud_user, cloud_id, status
        )
        if not switched:
        ...
        return (
            jsonify(
                {
                    "message": f"Error occurred while switching appliance status: {str(e)}"
                }
            ),
            500,
        )
...
def get_smartplugs(user_id):
    try:
        ...
        plugged_cloud_ids = [
            app.cloud_id
            for app in user.appliances
            if app.cloud_id and not app.is_deleted
        ]
        ...
        smart_plugs, error_message, status_code = cloud.get_smartplugs(
            plug_type, cloud_user
        )
        if not smart_plugs:
            return jsonify(error_message), status_code

        # Filter the smart plugs based on existing cloud IDs
        filtered_smart_plugs = [
            plug for plug in smart_plugs if plug.get("id") not in plugged_cloud_ids
        ]
        ...
        return (
            jsonify(
                {"message": f"Error occurred while retrieving smart plugs: {str(e)}"}
            ),
            500,
        )