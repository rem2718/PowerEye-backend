...
@pytest.mark.parametrize(
    ...
    (
        (
            NotifType.CREDS,
            None,
            ("Update Your Login", "Please update your login information for Meross."),
        ),
        (
            NotifType.PEAK,
            {"app_name": "tv"},
            (
                "Peak Usage Alert",
                "Try not to use tv after 5 PM. Click here to turn it off.",
            ),
        ),
    ),
)
...
def test_notify(fcm_instance):
    assert "powereye1-e599e" in fcm_instance.notify(
        "test user", NotifType.PEAK, {"app_name": "tv"}
    )
...
def test_notify(fcm_instance):
    ...
    fcm_instance.db.get_doc.assert_called_with(
        "Users", {"_id": user}, {"registration_token": 1}
    )
    fcm_instance.map_message.assert_called_with(NotifType.PEAK, {"app_name": "TestApp"})
    fcm_instance.logger.info.assert_called_with(
        "notify: NotifType.PEAK {'app_name': 'TestApp'}"
    )
    fcm_instance.messaging.send.assert_called_once()

    fcm_instance.logger.info.assert_any_call(
        "Notification sent with response: 'Test Response'"
    )
