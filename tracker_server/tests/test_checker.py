...
def test_get_powers(checker_instance, apps):
    expected_res = {
        "5f5b940f60a37c2b4012b973": 100,
        "5f5b940f60a37c2b4012b974": 8,
    }
    res = checker_instance._get_powers(apps)
...
def test_is_peak_hour(checker_instance, cur_hour, output):
    res = checker_instance._is_peak_hour(cur_hour)
    assert res == output
...
def test_get_model(checker_instance, model):
    res = checker_instance._get_model("64d1682993d44252699aa22a")
    assert pickle.dumps(res) == pickle.dumps(model)

    res = checker_instance._get_model("64d1682993d44252699aa32a")
    assert res == False
...
@pytest.mark.parametrize(
    ("flag_before", "is_peak_hour", "peak", "flag_after", "notified"),
    (
